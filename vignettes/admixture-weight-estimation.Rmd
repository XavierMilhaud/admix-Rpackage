...
The mixture weight $p$ of the unknown component distribution can be estimated using diverse techniques depending on the assumptions made on the unknown cdf $F$, among which the ones discussed in the sequel:
  ...
  * the estimator provided by Patra and Sen, [@PatraSen2016];
  * the estimator provided by Inversion - Best Matching method, [@MilhaudPommeretSalhiVandekerkhove2024a].
...
mixt1 <- twoComp_mixt(n = 800, weight = 0.7,
...
### Other cases
In this case, we use the Patra and Sen estimator, see [@PatraSen2016].
...
```{r}
## Simulate mixture data:
mixt1 <- twoComp_mixt(n = 1700, weight = 0.5,
                      comp.dist = list("norm", "norm"),
                      comp.param = list(list("mean" = 1, "sd" = 0.5),
                                        list("mean" = 0, "sd" = 1)))
mixt2 <- twoComp_mixt(n = 1500, weight = 0.7,
                      comp.dist = list("norm", "norm"),
...
In such a framework, it is thus better to use the estimator by [@PatraSen2016], which remains unbiased in full generality:
