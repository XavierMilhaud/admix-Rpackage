...
Estimation of the unknown quantities is made by the Inversion - Best Matching approach, see [@MilhaudIBM21].
```{r}
## Using Inversion - Best Matching method:
two_samples_test(sample1 = sample1, sample2 = sample2, comp.dist = list.comp, comp.param = list.param, 
                 method = 'ICV', n_sim_tab = 20, support = "Real", est.method = "PS", 
                 parallel = FALSE, n_cpu = 2)$decision
```
...
obj <- IBM_k_samples_test(samples = list(sim1,sim2,sim3), sim_U = NULL, n_sim_tab = 15, min_size = NULL,
                          comp.dist = list.comp, comp.param = list.param, parallel = FALSE, n_cpu = 2)
obj$rejection_rule
```

Now under the alternative hypothesis $H_1$ with $K=3$ populations:
```{r}
list.comp <- list(f1 = 'norm', g1 = 'norm',
                  f2 = 'norm', g2 = 'norm',
                  f3 = 'norm', g3 = 'norm')
list.param <- list(f1 = list(mean = 0, sd = 1), g1 = list(mean = 2, sd = 0.7),
                   ...
                   f3 = list(mean = 0, sd = 1), g3 = list(mean = 3, sd = 0.8))
sim1 <- rsimmix(n = 4000, unknownComp_weight = 0.8, comp.dist = list(list.comp$f1,list.comp$g1),
                comp.param = list(list.param$f1, list.param$g1))$mixt.data
sim2 <- rsimmix(n= 4300, unknownComp_weight = 0.6, comp.dist = list(list.comp$f2,list.comp$g2),
                comp.param = list(list.param$f2, list.param$g2))$mixt.data
sim3 <- rsimmix(n = 4100, unknownComp_weight = 0.7, comp.dist = list(list.comp$f3,list.comp$g3),
                comp.param = list(list.param$f3, list.param$g3))$mixt.data
## Perform the 3-samples test in a real-life setting:
list.comp <- list(f1 = NULL, g1 = 'norm',
                  f2 = NULL, g2 = 'norm',
